...
Follow [Architecture Remediation And Repository Governance Plan](docs/architecture/architecture-remediation-plan.md) before expanding product features. It covers the architecture audit, repository/Unit of Work correction, AppResponse, behaviours and caching, separate validators, operation-named command/handler files, and protected GitHub PR workflow. All implementation phases are pending; server-enforced rules for the private repository require eligibility verification.


Phase 0 progress: Git initialized from existing remote history; deployment gates and required CI added; main protection active. See [Phase 0 baseline](docs/development/phase-0-baseline.md). Local builds fail on existing Dividends repository calls and a Blazor syntax error; tests are compile-blocked. Import remains draft until remediation passes.
