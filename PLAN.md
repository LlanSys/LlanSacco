# LlanSacco - Living Architecture And Delivery Plan

> Last updated: 2026-08-28
>
> This file is the canonical roadmap for the LlanSacco solution.
...
LlanSacco is a multi-tenant SaaS platform for Kenyan Savings and Credit Co-operative Organizations (SACCOs), built on a production-grade .NET enterprise foundation with a **Production-First Mindset** from the get-go. The platform provides membership management, double-entry accounting, lending, HR/payroll, and regulatory compliance capabilities.
...
```
┌────────────────────────────────────────────────────────┐
│                      LS.UI.Rcl                         │ (Shared Razor Components)
└───────────┬────────────────────────────────┬───────────┘
            │ (HTTP REST)                    │ (HTTP REST)
┌───────────▼───────────┐        ┌───────────▼───────────┐
│     LS.UI.Blazor      │        │      LS.UI.Maui       │ (UI Hosts)
└───────────┬───────────┘        └───────────┬───────────┘
            └────────────────┬───────────────┘
                             │ (JSON over HTTP)
                    ┌────────▼────────┐
                    │     LS.Api      │ (Host Controller Gateway)
                    └────────┬────────┘
        ┌────────────────────┼────────────────────┐
┌───────▼───────┐    ┌───────▼───────┐    ┌───────▼───────┐
│LS.Infrastructure│  │LS.Persistence │    │LS.Application │ (Infrastructure / Data / Logic)
└───────┬───────┘    └───────┬───────┘    └───────┬───────┘
        │                    │                    │
        └────────────────────┼────────────────────┘
                             ▼
                     ┌───────────────┐
                     │   LS.Domain   │ (Core Entities & Business Rules)
                     └───────────────┘
```
...
4. **Compile-time Assembly/Namespace Checks:** `NetArchTest` checks in `LS.Tests.Architecture` enforce that code inside a module namespace (e.g., `LS.Application.Features.Banking`) does not reference classes in another module namespace (e.g., `LS.Application.Features.HR`) directly.

### 2.4 Feature Folder Convention
Code is organized horizontally by bounded context and vertically by feature:
```text
src/Backend/Application/LS.Application/Features/{BoundedContext}/{Feature}
```
...
- **Decoupled Frontend:** Blazor Web App (`LS.UI.Blazor`) restructured as a pure API consumer over HTTP (removes database dependencies, secrets, and MediatR references from the Web UI project).
- **Multi-Context Persistence:** Separate DbContexts (`IamDBContext`, `HrDBContext`, `BankingDBContext`, and `SharedDBContext`) created.
- **Shared Kernel & Validation:** Created `LS.SharedKernel` (DTOs) and `LS.SharedKernel.Validation` (Validators) to share validation logic between client-side Blazor/MAUI and backend MediatR behaviors.
...
```
  ...
  └──────────────────────────┬───────────────────────────────┘
                             ▼
  ┌──────────────────────────────────────────────────────────┐
  │ PHASE 6 - SACCO Product Features & UI Hardening          │
  └──────────────────────────┴──────────────────────────┘
```
...
### Phase 6 - SACCO Product Features And UI Hardening
*Goal: Transform the template into an enterprise-ready SACCO management platform with real financial workflows, maker-checker approvals, and module-specific dashboards.*

#### UI Hardening
- [x] **Per-Module Dashboards:** Each module (Membership, Accounting, Loans, HR) should have its own dashboard with relevant KPIs, charts, and quick actions populated from real data.
- [x] **Enterprise Auth Page Redesign:** Full visual redesign of authentication pages (split-screen with branded imagery, password recovery flow, registration flow) to enterprise-production quality.
- [x] **Responsive Mobile Layouts:** Verify and polish all pages for tablet and mobile form factors.

#### Core SACCO Features (Progressive Implementation)
- [x] **Membership:** Member onboarding, KYC verification, member status lifecycle, beneficiaries, next-of-kin.
- [x] **Shares:** Share capital management, share purchases, transfers, dividends calculation.
- [x] **Savings:** Savings accounts, interest calculation, withdrawal rules, minimum balance enforcement.
- [x] **Deposits:** Fixed and recurring deposit products, maturity tracking, early withdrawal penalties.
- [x] **Loans:** Loan products, loan applications, credit scoring, guarantors, disbursement, repayment schedules.
- [x] **FOSA:** Front Office Service Activity — over-the-counter transactions, cash management, teller operations.
- [x] **Collections:** Loan recovery, arrears tracking, follow-up workflows, collection officer assignment.
- [x] **Payroll Deductions:** Employer check-off integration, deduction schedules, remittance reconciliation.
- [x] **Dividends:** Dividend calculation, declaration, and distribution based on share capital and savings.

#### Financial Infrastructure
- [ ] **General Ledger:** Double-entry accounting, chart of accounts, journal entries, trial balance.
- [ ] **Accounting Reports:** Balance sheet, income statement, cash flow, regulatory reports.
- [ ] **M-Pesa Integration:** STK push for deposits/repayments, B2C for disbursements, statement reconciliation.
- [ ] **Banking Integrations:** Bank transfer APIs, RTGS/EFT, inter-bank reconciliation.

#### Workflow And Governance
- [ ] **Workflow/Approvals:** Maker-checker approval engine, configurable approval chains, escalation rules.
- [ ] **Audit Trail:** Comprehensive audit logging for all financial transactions and configuration changes.
- [ ] **Compliance:** Regulatory reporting (SASRA), anti-money laundering checks, suspicious transaction monitoring.
- [ ] **KYC:** Know Your Customer verification, document upload, ID validation, risk classification.

#### Member-Facing
- [ ] **Notifications:** Email, SMS, and in-app notifications for transactions, approvals, loan status.
- [ ] **Statements:** Member statements (savings, loans, shares), downloadable PDF/Excel.
- [ ] **Member Self-Service:** Member portal for balance inquiry, loan applications, statement requests.
- [ ] **Mobile/Web Portals:** Progressive web app or mobile-optimized portal for member access.
- [ ] **Reporting:** Management dashboards, trend analysis, portfolio-at-risk, delinquency reports.
...
- **Module Physical Split Trigger:** At what team or codebase size should folders in `LS.Domain` / `LS.Application` be physically split into individual project assemblies?
...
## Architecture Remediation Priority - 2026-09-10

Follow [Architecture Remediation And Repository Governance Plan](docs/architecture/architecture-remediation-plan.md) before expanding product features. It covers the architecture audit, repository/Unit of Work correction, AppResponse, behaviours and caching, separate validators, operation-named command/handler files, and protected GitHub PR workflow. Phase 0 is complete: the remote is now public (observed, not changed by this work), its main ruleset is active, and the failing baseline is recorded. Phase 1 is complete with 84 architecture tests and an exact shrinking debt register. Phase 2 is in progress; see [Phase 2 evidence](docs/architecture/phase-2-verification.md). Phases 3-7 remain pending.


Phase 0 and Phase 1 evidence is preserved in their reports. [Phase 1 PR #2](https://github.com/LlanSys/LlanSacco/pull/2) remains unmerged because Blazor cannot resolve ApplyForLoanRequest. [Phase 2 PR #3](https://github.com/LlanSys/LlanSacco/pull/3) is an in-progress draft covering persisted check-off validation, atomic payroll, savings/withdrawal persistence, retry cancellation/rollback, repeat-safe interest batches and provider-context tenant propagation verified on both providers. Durable dispatch, cross-context idempotency and the remaining mutation audit must pass before Phase 2 is complete. Required / Remediation remains enforced.
