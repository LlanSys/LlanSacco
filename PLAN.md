...
Follow [Architecture Remediation And Repository Governance Plan](docs/architecture/architecture-remediation-plan.md) before expanding product features. It covers the architecture audit, repository/Unit of Work correction, AppResponse, behaviours and caching, separate validators, operation-named command/handler files, and protected GitHub PR workflow. Phase 0 is complete: the remote is now public (observed, not changed by this work), its main ruleset is active, and the failing baseline is recorded. Phases 1-7 remain pending.


Phase 0 complete (bootstrap and baseline capture): Git initialized from existing remote history; deployment gates and required CI added; main protection active. See [Phase 0 baseline](docs/development/phase-0-baseline.md). Local builds fail on existing Dividends repository calls and a Blazor syntax error; tests are compile-blocked. [Draft import PR #1](https://github.com/LlanSys/LlanSacco/pull/1) remains blocked: GitHub reproduced the source failures and Required / Remediation failed; the secret scan passed. Next: Phase 1 standards and regression controls, including the minimum compilation repairs needed to execute architecture tests.
