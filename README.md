...
LlanSacco is a reusable enterprise .NET template for building production-minded systems from a modular monolith starting point.
...
4. Run `LS.Api` and `LS.UI.Blazor` from Visual Studio.
...
```powershell
dotnet test tests/LS.Tests.Unit/LS.Tests.Unit.csproj --no-restore -p:UseSharedCompilation=false
...
LlanSacco is reusable now for local-first application development when you:
