...
Ordinary handlers stage mutations and call CompleteAsync(ct) once. ExecuteInTransactionAsync(operation, ct) already owns saving/committing; do not call CompleteAsync redundantly inside it. Read-only generic methods use no tracking: explicitly stage detached mutations or use a tracked load. Pass cancellationToken explicitly to ExecuteInTransactionWithRetryAsync. The retry callback must re-read mutable state on each attempt, stage changes and let the helper save once. Failed attempts discard tracked state; a returned failure envelope does not itself roll back intentional writes. Do not publish irreversible external side effects from a retry callback. Domain-event consistency across commit paths remains tracked in remediation Phase 2.
