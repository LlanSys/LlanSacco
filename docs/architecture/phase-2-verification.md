...
## Password reset, accounting upgrades and FOSA audit - 2026-09-13

Password reset now starts the IAM transaction before calling Identity. Identity's scoped IamDBContext shares the transaction, so password/hash/stamp changes, reset metadata and token revocation commit together. Each retry reloads the user. The handler checks the metadata-update result, propagates request cancellation, and passes cancellation to token queries and staged revocations.

The retry helper accepts an optional shouldCommit predicate. A rejected predicate rolls back already executed embedded saves, clears tracked state and returns the expected result. Existing callers retain their intentional failure-result commits unless they opt in. Password reset uses response.IsSuccess. This handles Identity returning a failure after its password save without using exceptions for an expected business result. Canonical instructions and persistence standards document the choice.

Ten password-reset provider cases cover success, revocation failure, concurrency retry, cancellation and Identity metadata rejection after the password save. Fresh contexts verify that unsuccessful transactions preserve the old password hash, security stamp and reset timestamp and leave the token unrevoked. Two additional provider cases prove explicit rejection rolls back an embedded save; existing tests still prove intentional diagnostics commit by default.

Separate RepairAccountingUniqueIndexes migrations rebuild account-code, journal-reference and transaction-type-mapping uniqueness for PostgreSQL and SQL Server. They use standard EF DropIndex/CreateIndex operations and the repaired generators. Four upgrade tests first apply the previous full migration chain, replace these indexes with non-unique versions bearing the same names, and then upgrade. Clean data regains all three unique indexes. Duplicate journals prevent upgrade; the migration transaction restores the old indexes, preserves both records and leaves the migration unapplied. Down retains uniqueness instead of deliberately weakening financial invariants. No deployed database was migrated, and no duplicate financial data is deleted automatically.

The FOSA audit found that negative/over-precision cash amounts, unsupported transaction types and negative denomination counts could reach balance writers without effective validation. OTC processing and vault transfers now reject these before mutation; OTC records use the current stable actor identifier. Two provider tests verify rejected requests leave no transaction or balance changes, successful withdrawals/vault transfers conserve balances, and a stale competing update cannot overwrite the committed account/till state. Broader FOSA idempotency and transfer-history coverage remain open.
