...
This document defines how LlanSacco should model SaaS tenancy, deployment isolation, and environment configuration before cloning the template for client products.

## Decision

LlanSacco uses a hybrid deployment-stamp SaaS model.

- A tenant is a customer organization, business unit, or client workspace served by a product built from LlanSacco.
...
LlanSacco should separate SaaS control-plane concerns from product data-plane concerns.
...
LlanSacco currently supports the pooled-database model through tenant-scoped entities and global query filters.
...
```text
LlanSacco API image:v1.2.0 -> aca-tenant-a with Tenant A configuration
LlanSacco API image:v1.2.0 -> aca-tenant-b with Tenant B configuration
LlanSacco API image:v1.2.0 -> aca-custom-client with custom provider configuration
```
