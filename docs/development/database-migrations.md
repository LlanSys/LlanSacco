...
LlanSacco supports multiple database providers simultaneously (e.g., SQL Server and PostgreSQL) within a modular monolith architecture. This means each bounded context (IAM, Banking, HR, Shared, etc.) has its own set of migrations, and each set of migrations must be generated for each supported provider.
...
dotnet ef migrations add <MigrationName> `
  ...
  --project src\Backend\Persistence\LS.Persistence\LS.Persistence.csproj `
  --startup-project src\Backend\Api\LS.Api\LS.Api.csproj
```
