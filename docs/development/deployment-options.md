...
This document explains the supported deployment paths for LlanSacco and cloned products. It keeps Azure-ready work intact while allowing local-first and non-Azure deployment when Azure credits or subscriptions are unavailable.
...
- Use managed Postgres/Redis only if the cloned application can use them. LlanSacco currently uses SQL Server, so a SQL Server-compatible external database or a VPS-based SQL Server plan must be decided separately.
...
- Runs `LS.Api` and `LS.UI.Blazor` from Visual Studio.
...
LlanSacco uses GHCR as the first neutral image registry because it is already attached to GitHub Actions and does not require Azure credits.
...
- The images are `ghcr.io/<owner>/llan-sacco-api` and `ghcr.io/<owner>/llan-sacco-blazor`.
