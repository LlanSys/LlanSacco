...
This checklist explains how LlanSacco should be configured locally, when developing locally against Azure services, when deployed to Azure, and when deployed to supported non-Azure container hosts.
...
LlanSacco uses `ASPNETCORE_ENVIRONMENT` for application behavior and normal .NET configuration precedence for values.
...
```powershell
dotnet user-secrets set "ConnectionStrings:DefaultConnection" "<local-sql-connection>" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets set "JwtSettings:Secret" "<strong-local-jwt-secret>" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets set "IamProvisioning:TemporaryPassword" "<temporary-password>" --project src\Backend\Api\LS.Api\LS.Api.csproj
```
...
```powershell
dotnet user-secrets set "PasswordRecovery:Mode" "EmailOtp" --project src\Backend\Api\LS.Api\LS.Api.csproj

# Alternative deployment mode:
dotnet user-secrets set "PasswordRecovery:Mode" "EmailLink" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets set "EmailSettings:ClientBaseUrl" "https://localhost:7049" --project src\Backend\Api\LS.Api\LS.Api.csproj
```

Local email should normally be captured by Mailpit through the setup script:

```powershell
dotnet user-secrets set "EmailSettings:Provider" "LocalMailpit" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets set "EmailSettings:LocalMailpit:Host" "localhost" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets set "EmailSettings:LocalMailpit:Port" "1025" --project src\Backend\Api\LS.Api\LS.Api.csproj
```
...
```powershell
dotnet user-secrets set "Payments:Provider" "NoOp" --project src\Backend\Api\LS.Api\LS.Api.csproj

# Stripe
dotnet user-secrets set "Payments:Provider" "Stripe" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets set "Payments:Stripe:SecretKey" "<stripe-secret-key>" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets set "Payments:Stripe:WebhookSigningSecret" "<stripe-webhook-signing-secret>" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets set "Payments:Stripe:SuccessUrl" "https://localhost:7049/payments/success" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets set "Payments:Stripe:CancelUrl" "https://localhost:7049/payments/cancel" --project src\Backend\Api\LS.Api\LS.Api.csproj

# M-Pesa Daraja
dotnet user-secrets set "Payments:Provider" "Mpesa" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets set "Payments:Mpesa:ConsumerKey" "<daraja-consumer-key>" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets set "Payments:Mpesa:ConsumerSecret" "<daraja-consumer-secret>" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets set "Payments:Mpesa:ShortCode" "<paybill-or-till>" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets set "Payments:Mpesa:PassKey" "<daraja-passkey>" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets set "Payments:Mpesa:AuthEndpoint" "https://sandbox.safaricom.co.ke/oauth/v1/generate?grant_type=client_credentials" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets set "Payments:Mpesa:StkPushEndpoint" "https://sandbox.safaricom.co.ke/mpesa/stkpush/v1/processrequest" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets set "Payments:Mpesa:StkQueryEndpoint" "https://sandbox.safaricom.co.ke/mpesa/stkpushquery/v1/query" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets set "Payments:Mpesa:CallbackUrl" "https://<public-callback-host>/api/v1/payments/mpesa/callback" --project src\Backend\Api\LS.Api\LS.Api.csproj
```
...
```powershell
dotnet user-secrets set "EntraId:Enabled" "true" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets set "EntraId:TenantId" "<tenant-id>" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets set "EntraId:ClientId" "<app-client-id>" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets set "EntraId:ClientSecret" "<app-client-secret>" --project src\Backend\Api\LS.Api\LS.Api.csproj
```

## Provider Readiness Without Enabling Providers

LlanSacco keeps provider integrations code-ready and configuration-ready without forcing every cloned application to use every provider.
...
```powershell
dotnet user-secrets remove "DataProtection:BlobKeyUri" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets remove "DataProtection:KeyVaultKeyIdentifier" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets set "ProfileImageStorage:Provider" "Local" --project src\Backend\Api\LS.Api\LS.Api.csproj
...
```powershell
dotnet user-secrets set "DataProtection:KeyEncryptionMode" "KeyVault" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets set "DataProtection:BlobKeyUri" "https://<storage-account>.blob.core.windows.net/dataprotection-keys/keyring.xml" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets set "DataProtection:KeyVaultKeyIdentifier" "https://<vault>.vault.azure.net/keys/<key>/<version>" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets set "ProfileImageStorage:Provider" "AzureBlob" --project src\Backend\Api\LS.Api\LS.Api.csproj
dotnet user-secrets set "ProfileImageStorage:AzureBlob:ContainerUri" "https://<storage-account>.blob.core.windows.net/profile-images" --project src\Backend\Api\LS.Api\LS.Api.csproj
```
...
DataProtection__ApplicationName=LlanSacco
...
EmailSettings__DisplayName=LlanSacco
...
## Remediation Deployment Gate

Deployment and provisioning workflows are manual-only and require the repository variable `LLANSACCO_DEPLOYMENT_ENABLED=true`. Container image publishing additionally requires manual dispatch with publish_images and `LLANSACCO_PUBLISH_ENABLED=true`. Leave both unset during source import. Do not enable them until the LlanSacco target resources, OIDC identity, environment controls, and rollback procedure are verified. CI and local verification use `scripts/verify-baseline.ps1`; no application host is started by this script.

Initial import moves non-empty development passwords, provider keys, license keys, and connection strings into the API user-secrets store. New developers must populate these locally; committed Development settings retain empty fields. Existing user-secret overrides are preserved.
