...
LlanSacco runs application code from Visual Studio and runs replaceable infrastructure dependencies in Docker. This preserves normal .NET debugging while making a new developer machine reproducible.

Docker Desktop groups these services under the Compose project name `cg-llansacco`. The `cg-` prefix identifies the row as a container group rather than an individual service.

Data volumes retain stable `llansacco_*` names independently of the group label. The setup script creates missing volumes and reuses existing ones, allowing container-group renames and container recreation without abandoning local data.
...
Run the script from the repository root, the folder containing `LlanSacco.sln`, `AGENTS.md`, and the `scripts` folder:
...
The SQL password in `APP_SQL_CONNECTION_STRING` is not the LlanSacco sign-in password. The development admin user is seeded from `DevelopmentSeed:AdminEmail` and `DevelopmentSeed:AdminPassword`; by default that is `aamodhiambo@gmail.com` / `Admin@12345`.
...
- Builds `LS.Api` from `ops/docker/api.Dockerfile`.
- Builds `LS.UI.Blazor` from `ops/docker/blazor.Dockerfile`.
...
If an authenticator enrollment is reset this way, delete the old LlanSacco account entry from Microsoft Authenticator/Google Authenticator/1Password and scan the new QR code. Old 6-digit codes are tied to the old secret and will remain invalid.
...
LlanSacco pins MassTransit `8.5.10`, which targets .NET 10 and is published under Apache-2.0. MassTransit v9 is a commercial release and is not used by the reusable template. Revisit this decision deliberately if a future application purchases MassTransit v9 support and features.
