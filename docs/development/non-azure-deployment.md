...
This guide covers the non-Azure deployment paths for LlanSacco and cloned products. Azure remains the preferred enterprise path when the subscription is active, but the template also supports container deployment to DigitalOcean, Heroku, and generic Docker hosts.
...
| Concern | Azure | DigitalOcean | Heroku | LlanSacco setting |
| --- | --- | --- | --- | --- |
...
| `HEROKU_API_APP_NAME` | Variable | Heroku app name for `LS.Api` |
| `HEROKU_BLAZOR_APP_NAME` | Variable | Heroku app name for `LS.UI.Blazor` |
...
LlanSacco reads Heroku's assigned `PORT` environment variable at startup. This is required because Heroku does not guarantee a fixed container port.
