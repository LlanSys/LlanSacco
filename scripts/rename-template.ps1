...
function Get-ReplacedText([string]$value) {
    $updated = $value.Replace('LlanSacco', $displayName)
    $updated = $updated.Replace('LlanSacco', $compactName)
    $updated = $updated.Replace('llan-sacco', $kebabName)
    $updated = $updated.Replace('llansacco', $lowerCompactName)
    ...
    $updated = $updated.Replace('LSApi', "${NamespacePrefix}Api")
