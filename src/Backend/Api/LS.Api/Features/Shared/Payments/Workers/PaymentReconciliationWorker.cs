...
namespace LS.Api.Features.Shared.Payments.Workers;
    ...
    ILogger<PaymentReconciliationWorker> logger) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                await ReconcilePendingPaymentsAsync(stoppingToken).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
    ...
    private async Task ReconcilePendingPaymentsAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var record in pendingRecords)
        {
            ...
            try
            {
                var gatewayResponse = await paymentGateway.GetStatusAsync(record.ProviderReference, record.Provider, cancellationToken).ConfigureAwait(false);
