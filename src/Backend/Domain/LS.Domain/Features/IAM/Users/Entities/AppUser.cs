...
public class AppUser : IdentityUser, ISoftDeletable, IHasDomainEvents
{
    ...
    // Source of truth for these remains Employee/Member
    ...
    // Factory for Member-linked user
    public static AppUser CreateForMember(
        Guid tenantId,
        Guid memberId,
        string userName,
        string firstName,
    ...
    public void LinkToMember(Guid memberId)
    {
        if (MemberId.HasValue)
            throw new DomainException("User is already linked to a member record.");

        MemberId = memberId;
        RaiseDomainEvent(new MemberLinkedToUserEvent(Id, memberId));
    }
    ...
    public void GrantAccess(string grantedBy, IEnumerable<string> defaultRoles)
    {
        ...
        RaiseDomainEvent(new AppUserAccessGrantedEvent(Id, EmployeeId, MemberId, grantedBy));
    }
    ...
    public void RevokeAccess(string revokedBy, string reason)
    {
        ...
        RaiseDomainEvent(new AppUserAccessRevokedEvent(Id, EmployeeId, MemberId, revokedBy, reason));
    }
}
