...
namespace LS.Infrastructure.Contracts.Implementations.Caching;

internal sealed class HybridCacheService(HybridCache cache, ILogger<HybridCacheService> logger) : ICacheService
{
    private static readonly ActivitySource CacheActivity = new("LS.Cache");
    ...
    public async Task<T?> GetAsync<T>(string key, CancellationToken ct = default)
    {
        ...
        try
        {
            //ToDo: HybridCache does not have a TryGetAsync. We can use GetOrCreateAsync with a factory that returns a recognizable not-found value if we wanted, but often we just rely on IDistributedCache if needed. 
