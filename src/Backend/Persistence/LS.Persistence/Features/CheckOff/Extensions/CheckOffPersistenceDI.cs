...
namespace LS.Persistence.Features.CheckOff.Extensions;

public static class CheckOffPersistenceDI
{
    public static IServiceCollection AddCheckOffPersistence(this IServiceCollection services, IConfiguration configuration)
    {
        // Using BankingConnection or DefaultConnection depending on available configuration for checkoffs.
        ...
        void ConfigureDbContextOptions(IServiceProvider provider, DbContextOptionsBuilder options)
        {
            if (dbSettings.Provider.Equals("PostgreSql", StringComparison.OrdinalIgnoreCase))
            {
                options.UseNpgsql(connectionString, pgOptions =>
                {
                    ...
                    pgOptions.MigrationsHistoryTable("__EFMigrationsHistory_CheckOff");
            ...
            else
            {
                options.UseSqlServer(connectionString, sqlOptions =>
                {
                    ...
                    sqlOptions.MigrationsHistoryTable("__EFMigrationsHistory_CheckOff");
        ...
        if (dbSettings.Provider.Equals("PostgreSql", StringComparison.OrdinalIgnoreCase))
        {
            services.AddDbContext<CheckOffDBContext, CheckOffPostgreSqlDBContext>(ConfigureDbContextOptions);
        }
        else
        {
            services.AddDbContext<CheckOffDBContext, CheckOffSqlServerDBContext>(ConfigureDbContextOptions);
        }

        services.AddScoped<ICheckOffUnitOfWork, CheckOffUnitOfWork>();
