...
public class LoansDBContext : DbContext, ITenantFilteredDBContext
{
    private readonly ICurrentTenantProvider? _tenantProvider;
    private readonly ICurrentActorProvider? _actorProvider;
    private readonly ILogger<LoansDBContext>? _logger;

    public LoansDBContext(
        ...
        ILogger<LoansDBContext>? logger = null
    ) : base(options)
    {
    ...
    public DbSet<CollectionAction> CollectionActions { get; set; }
    public DbSet<CollectionPromise> CollectionPromises { get; set; }
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.ApplyConfigurationsFromAssembly(
            ...
            type => type.Namespace?.StartsWith("LS.Persistence.Features.Loans", StringComparison.Ordinal) == true &&
    ...
    public override async Task<int> SaveChangesAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            if (_logger is not null)
                PersistenceLogDefinitions.LogDBContextSaveChangesError(_logger, nameof(LoansDBContext), ex);
            _collectedDomainEvents?.Clear();
            throw;
        }
    }
