...
public sealed class NamingConventionTests
{
    ...
    public void Dtos_Should_Reside_In_Shared_Project()
    {
        ...
        // DTOs belong exclusively in LS.Shared.
        ...
        domainDtos.Should().BeEmpty(
            because: "DTOs must live in LS.Shared, not LS.Domain. " +
                     "Found in Domain: {0}", string.Join(", ", domainDtos.Select(t => t.Name)));

        persistenceDtos.Should().BeEmpty(
            because: "DTOs must live in LS.Shared, not LS.Persistence. " +
    ...
    public void SharedKernel_FeatureOwned_Types_Should_Reside_In_Features_Namespace()
    {
        var misplacedTypes = Types.InAssembly(AssemblyReferences.SharedKernel)
            .That()
            .ResideInNamespaceStartingWith("LS.SharedKernel.Dtos.Auth")
            .Or()
            .ResideInNamespaceStartingWith("LS.SharedKernel.Dtos.Employees")
            .Or()
            .ResideInNamespaceStartingWith("LS.SharedKernel.Dtos.Directors")
            .Or()
            .ResideInNamespaceStartingWith("LS.SharedKernel.Dtos.Dashboard")
            .Or()
            .ResideInNamespaceStartingWith("LS.SharedKernel.Dtos.Lookups")
            .Or()
            .ResideInNamespaceStartingWith("LS.SharedKernel.Dtos.Accounting")
            .Or()
            .ResideInNamespaceStartingWith("LS.SharedKernel.Enums")
            .GetTypes();

        misplacedTypes.Should().BeEmpty(
            because: "feature-owned SharedKernel DTOs and enums must live under LS.SharedKernel.Features.*. Found: {0}",
    ...
    public void SharedKernel_Validators_Should_Reside_In_Features_Or_Common_Namespace()
    {
        ...
        var misplacedTypes = validatorTypes
            .Where(t => t.Namespace is null ||
                        (!t.Namespace.StartsWith("LS.SharedKernel.Validation.Features.", StringComparison.Ordinal) &&
                         !t.Namespace.StartsWith("LS.SharedKernel.Validation.Validators.Common", StringComparison.Ordinal)))
            .Select(t => t.FullName)
            .ToList();

        misplacedTypes.Should().BeEmpty(
            because: "feature-owned validators must live under LS.SharedKernel.Validation.Features.*; only generic validator bases stay in Common. Found: {0}",
    ...
    public void Api_Controllers_Should_Reside_In_Features_Namespace()
    {
        ...
        misplacedControllers.Should().BeEmpty(
            because: "feature-owned API controllers must live under LS.Api.Features by bounded context and feature. Found: {0}",
    ...
    public void Custom_Context_Related_Types_Should_Use_DB_Acronym()
    {
        ...
        var violations = assemblies
            ...
            .Where(static type => type.Namespace is not null &&
                                  type.Namespace.StartsWith("LS.", StringComparison.Ordinal))
