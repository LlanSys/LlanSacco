...
public sealed class ProjectDependencyTests
{
    ...
    private const string NsDomain = "LS.Domain";
    private const string NsApplication = "LS.Application";
    private const string NsPersistence = "LS.Persistence";
    private const string NsInfrastructure = "LS.Infrastructure";
    private const string NsSharedKernel = "LS.SharedKernel";
    private const string NsApi = "LS.Api";

    private static readonly string[] AccountingForbiddenDependencies = ["LS.Application.Features.HR", "LS.Application.Features.IAM"];
    private static readonly string[] HrForbiddenDependencies = ["LS.Application.Features.Accounting", "LS.Application.Features.IAM"];
    private static readonly string[] IamForbiddenDependencies = ["LS.Application.Features.Accounting", "LS.Application.Features.HR"];
    ...
    [Fact]
    public void Accounting_Module_ShouldNot_DependOn_HR_Or_IAM()
    {
        var result = Types.InAssembly(AssemblyReferences.Application)
            .That()
            .ResideInNamespace("LS.Application.Features.Accounting")
            .ShouldNot()
            .HaveDependencyOnAny(AccountingForbiddenDependencies)
            .GetResult();
        ...
        result.IsSuccessful.Should().BeTrue(
            because: $"Accounting feature module must not depend on HR or IAM modules directly. Failing types: {failingTypeNames}");
    }

    [Fact]
    public void HR_Module_ShouldNot_DependOn_Accounting_Or_IAM()
    {
        var result = Types.InAssembly(AssemblyReferences.Application)
            .That()
            .ResideInNamespace("LS.Application.Features.HR")
            .ShouldNot()
        ...
        result.IsSuccessful.Should().BeTrue(
            because: $"HR feature module must not depend on Accounting or IAM modules directly. Failing types: {failingTypeNames}");
    }

    [Fact]
    public void IAM_Module_ShouldNot_DependOn_Accounting_Or_HR()
    {
        var result = Types.InAssembly(AssemblyReferences.Application)
            .That()
            .ResideInNamespace("LS.Application.Features.IAM")
            .ShouldNot()
        ...
        result.IsSuccessful.Should().BeTrue(
            because: $"IAM feature module must not depend on Accounting or HR modules directly. Failing types: {failingTypeNames}");
    }
