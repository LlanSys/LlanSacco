...
using LS.Application.Features.IAM.Users.Validators;
using LS.SharedKernel.Dtos.Common;
using LS.SharedKernel.Dtos.Utilities;
using LS.SharedKernel.Features.Shared.Common.Enums;
using LS.SharedKernel.Features.IAM.Users.Dtos;

using MediatR;

namespace LS.Tests.Unit.Application.Validation;
...
public sealed class ValidationBehaviorTests
{
    ...
    public async Task Handle_Should_Call_Next_When_Command_Is_Valid()
    {
        var validator = new LoginCommandValidator();
        var behavior = new ValidationBehavior<LoginCommand, AppResponse<LoginResponse>>([validator]);
        var command = new LoginCommand(new TestLoginRequest("admin@llansacco.local", "Password1!", false, null, "device-1"));
